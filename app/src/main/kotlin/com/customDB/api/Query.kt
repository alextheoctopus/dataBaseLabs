...
object Q {
    ...
    fun gte(field: String, value: Number): Predicate = Predicate {
        (it.values[field] as? Number)?.toDouble()?.let { v -> v >= value.toDouble() } == true
    }
    fun lt(field: String, value: Number): Predicate = Predicate {
        (it.values[field] as? Number)?.toDouble()?.let { v -> v < value.toDouble() } == true
    }
    fun lte(field: String, value: Number): Predicate = Predicate {
        (it.values[field] as? Number)?.toDouble()?.let { v -> v <= value.toDouble() } == true
    }
