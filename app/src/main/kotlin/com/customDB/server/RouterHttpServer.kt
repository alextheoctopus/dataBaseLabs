...
class RouterHttpServer(
    ...
    fun start(port: Int) {
        ...
        srv.createContext("/execute") { ex ->
            ...
            val shard = locator.shardIdFor(sql)
            val leader = cluster.leaderOf(shard)
            val resp = forward(leader, "/execute", sql.toByteArray())
        ...
        srv.createContext("/query") { ex ->
            val sql = ex.requestBody.readAllBytes().toString(StandardCharsets.UTF_8)
            val shard = locator.shardIdFor(sql)
            val node = cluster.bestReplicaOrLeader(shard)
