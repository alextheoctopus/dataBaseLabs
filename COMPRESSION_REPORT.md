...
- `GzipFormat.kt` использует стандартный `java.util.zip.GZIPOutputStream` для создания GZIP файлов
...
├── LZ77.kt                    - Алгоритм LZ77 (самостоятельная реализация)
├── LZ77Encoder.kt             - Бинарный формат для LZ77 токенов (наш формат)
├── CustomCompressionFormat.kt - Главный интерфейс (LZ77 + LZ77Encoder)
├── GzipFormat.kt              - GZIP формат (использует стандартный GZIPOutputStream для совместимости с gunzip)
├── DeflateEncoder.kt          - Не используется (оставлен для истории)
├── BitWriter.kt               - Писатель битов (не используется)
└── BitReader.kt                - Читатель битов (не используется)

app/src/main/kotlin/com/customDB/api/
└── Codec.kt                   - Интеграция сжатия в базу данных (использует CustomCompressionFormat)

app/src/test/kotlin/com/customDB/
└── DatabaseCompressionTest.kt - Тесты интеграции сжатия с БД
```
...
✅ **GZIP формат реализован** для совместимости с внешними утилитами (gunzip)
...
- Полная совместимость с gunzip для GZIP файлов (для демонстрации)
- **Разделение:** `CustomCompressionFormat` используется в БД, `GzipFormat` только для демонстрации gunzip совместимости
